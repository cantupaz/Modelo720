...
class Detalle720(BaseModel):
    ...
    def validate_detail_rules(self):
        ...
        if self.clave_tipo_bien == ClaveBien.C and self.clave_ident_cuenta not in ("I", "O", " ", ""):
            raise ValueError("clave_ident_cuenta must be 'I', 'O', or blank for clave_tipo_bien 'C'")
        
        if self.clave_tipo_bien == ClaveBien.B and self.clave_tipo_bien_inmueble not in ("U", "R", " ", ""):
            raise ValueError("clave_tipo_bien_inmueble must be 'U', 'R', or blank for clave_tipo_bien 'B'")
